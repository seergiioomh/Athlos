...
export function CoachInsightCard({
  ...
  return <Pressable onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
    {/* Sin círculo detrás: el gris cortaba el violeta de la tarjeta justo en
        la esquina que más se mira. La chispa sola se lee igual de bien. */}
    <HugeiconsIcon icon={SparklesIcon} color={HomeColors.purple} size={26} strokeWidth={2.2} />
...
const styles = StyleSheet.create({
  card: { marginTop: 20, minHeight: 128, padding: 20, borderRadius: 24, backgroundColor: HomeColors.purpleTinted, borderWidth: 1, borderColor: HomeColors.purpleSoft, flexDirection: "row", alignItems: "flex-start", gap: 12 }, pressed: { opacity: 0.78 },
  copy: { flex: 1, gap: 4 },
